...
def plot_delivery_data(df):
    ...
    norm = Normalize(
        vmin=df["time_of_day_hours"].min(), vmax=df["time_of_day_hours"].max()
    )
    ...
    # Grid lines styling
    ax.grid(
        True, which="both", linestyle="--", linewidth=0.5, color=grid_color, alpha=0.5
    )
...
def plot_rush_hour(data_df, features):
    ...
    rush_hour_handle = Line2D(
        [0],
        [0],
        marker="o",
        color="w",
        label="Rush Hour",
        markerfacecolor=rush_hour_color,
        markersize=10,
        # Edge color matches fill color
    ...
    legend = ax.legend(
        handles=[rush_hour_handle, not_rush_hour_handle],
...
def plot_final_data(features, targets):
    ...
    weekday_non_rush_handle = Line2D(
        [0],
        [0],
        marker="o",
        color="w",
    ...
    weekend_non_rush_handle = Line2D(
        [0],
        [0],
        marker="o",
        color="w",
...
def plot_model_predictions(predicted_outputs, actual_targets):
    ...
    # Custom Legend Configuration
    model_pred_handle = Line2D(
        [0],
        [0],
        marker="o",
        color="w",
        markerfacecolor=model_predictions_color,
        markersize=10,
        linestyle="None",
    )
    perfect_pred_handle = Line2D(
        [0],
        [0],
        color=perfect_prediction_line_color,
        linewidth=2,
...
def prediction(model, data_df, raw_inputs, rush_hour_feature_func):
    ...
    if distance_value <= 0:
        print(
            "\033[91mError: Invalid distance value - The value must be greater than 0 and no more than 20 miles."
        )
        return
    ...
    if time_value <= 0 or time_value > 24:
        print(
            "\033[91mError: Invalid time value - The value must be greater than 0 and no more than 24.0."
        )
        return
    ...
    if weekend_value not in [0.0, 1.0]:
        print(
            "\033[91mError: Invalid weekend value - Please use 1 (True) for a weekend or 0 (False) for a weekday."
        )
        return
    ...
    if distance_value > 20:
        print(
            f"\033[91mDelivery Area Exceeded: The requested distance of {distance_value:.1f} miles exceeds the 20-mile limit."
        )
        return
    ...
    if time_value <= 8 or time_value > 20:
        print(
            "\033[91mOutside Operating Hours: Deliveries are only processed between 8:00 and 20:00."
        )
        return
    ...
    line4 = (
        f"| {'Is this considered a rush hour period?':<40} | {is_rush_hour_text:<21} |"
    )
    final_result = (
        f"| {'Estimated Delivery Time':<40} | {f'{predicted_time:.2f} minutes':<21} |"
    )
